...
GameBoard.prototype.readyBoardForOutput = function() {
  var board = []
    ...
    if (colorPieces[i].alive) {
      ...
      for (var rel = 0; rel < localCheck.length; ++rel) {
        ...
        if (killSpot >= 0 && this.spaceOccupied(emptyCoordCheck, oppColorPieces) === -1 &&
          ...
          // *** This allows a piece to become a king and make another jump if ***
          // *** one is availabe, which follows international rules, but not American rules ***
...
GameBoard.prototype.checkValidMoveDistance = function (color, selected, teamArr) {
	var greaterThanMaxMovement = (color === "red") ? game.selectedSquare.row - game.prevSelectedSquare.row > 1 : game.selectedSquare.row - game.prevSelectedSquare.row < -1;
	if (selected > 0 && teamArr[selected].isKing)
		greaterThanMaxMovement = Math.abs(game.selectedSquare.row - game.prevSelectedSquare.row) > 1;
	if (!greaterThanMaxMovement) {
		greaterThanMaxMovement = Math.abs(game.selectedSquare.col - game.prevSelectedSquare.col) > 1;
	}
	if (greaterThanMaxMovement) {
		return false;
	}
  ...
  if (game.lastMouseClick.x != -1) {
    ...
    if (selected === -1 && oppselect === -1 && game.selectedSquare.index != -1) {
      ...
      var chosenJumpPath = false;
			...
			if (this.checkValidMoveDistance(color, selected, teamArr) === false && possibleJumps.length === 0)
				return;
      ...
      if (possibleJumps.length === 0 || chosenJumpPath) {
...
GameBoard.prototype.resetClickState = function() {
  game.lastMouseClick.x = -1;
  game.selectedSquare.index = -1;
}

GameBoard.prototype.checkIfOnBlackTile = function(loc) {
  if (loc.row % 2 === 0 && loc.col % 2 === 0 || loc.row % 2 === 1 && loc.col % 2 === 1)
    return true;
  return false;
}

GameBoard.prototype.checkIfOutOfBounds = function(loc) {
  if (loc.row >= game.tileDim || loc.col >= game.tileDim || loc.row < 0 || loc.col < 0)
    return true;
  return false;
}

GameBoard.prototype.checkIfMoveInvalid = function(loc) {
  if (this.checkIfOnBlackTile(loc) === false || this.checkIfOutOfBounds(loc) === true)
    return true;
  return false;
}
...
GameBoard.prototype.spaceOccupied = function(loc, arr) {
  if (this.checkIfMoveInvalid(loc) === true)
    return -2;
