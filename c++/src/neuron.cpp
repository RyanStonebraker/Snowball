...
Neuron& Neuron::evolveChild(){
    Neuron evolvedChild;
    float evolvedWeight = this->nodeWeightPrime(this->getWeight(), this->getRiskFactor());
    float evolvedSigmaWeight = this->sigmaWeightPrime(this->getRiskFactor(), 1);
    float evolvedKingWeight = this->kingWeightPrime(this->getAverageWeight());

    evolvedChild.setWeight(evolvedWeight);
    evolvedChild.setAverageWeight(evolvedSigmaWeight);
    evolvedChild.setRiskFactor(evolvedKingWeight);
    return evolvedChild;
}
