...
string flipColor (const string & board) {
	string flippedBoard = "";

	for (int piece = board.size() - 1; piece >= 0; --piece) {
		if (piece == BLACK)
			flippedBoard += '1';
		else if (piece == RED)
			flippedBoard += '2';
...
>>>>>>> e256622578d3a563cd82e68604c04b9697c0dec9
...
int main(int argc, char* argv[])
{
	...
	uniform_real_distribution<> rand_weight(-1, 1);
...
>>>>>>> e256622578d3a563cd82e68604c04b9697c0dec9
	...
	if (weightFile != "") {
		ifstream readWeights(weightFile);

		readWeights >> currentWeights;
	} else {
		std::ofstream weightWriter("../comm/tempWeights.txt");
		weightWriter << currentWeights;
	}
